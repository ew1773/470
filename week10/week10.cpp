...
// levels of secrecy
enum Control { PUBLIC, STUDENT, GRADER, PROFESSOR};
 ...
 * Display scores
 ***********************************************/
void StudentGrade::displayScores(User userSignedIn)
{
   if (scores.size() == 0)
      return;
 ...
 ***********************************************/
void Interface::interact(User userSignedIn) 
{
   int iSelected;
   while (-1 != (iSelected = promptForStudent()))
   {
      ...
      // show the results
      students[iSelected].displayScores(userSignedIn);
...
int main()
{
   ...
   interface.interact(userSignedIn);
